...
console.log(carouselRef?.current?.offsetWidth,"offsetwidth");
 ...
 if(carouselRef.current&&elemRef?.current?.offsetWidth){
      ...
      if(carouselRef.current?.scrollLeft===0){
          ...
          carouselRef.current.scrollLeft=((carouselRef?.current?.offsetWidth)+(elemRef.current.offsetWidth*2))
          ...
          carouselRef.current.classList.add('no-transition')
          carouselRef.current.scrollLeft=(carouselRef?.current?.offsetWidth+(elemRef.current.offsetWidth*4))
          carouselRef.current.classList.remove('no-transition')
      }
  }
 ...
 onMouseMove={handleMouseMove}
           className="carousel-h-slider bg-stone-200">
            {
