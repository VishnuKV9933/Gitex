...
export default function  Emailvalidater({  email,confirmEmail,alert,setConEmail,setEmail}: RegisterFormInputProps) :React.ReactElement{
...
useEffect(()=>{
    ...
    const emalhandleInputBlur = ():void => {
      ...
      const handleConEmailChange = (event:React.ChangeEvent<HTMLInputElement>):void => {
  ...
  <input
   ...
   className="w-full form-input h-6 " name='email' type="text" />

  <>
  {(!emailvalid&&email)&&
  <div  style={{ fontSize: '11px' }} className='text-red-500 font-semibold'>Email not valid</div>
  }
 ...
 {!match&&
  <div  style={{ fontSize: '11px' }} className='text-red-500 font-semibold'> Email addresses do not match</div>
  }

  <>
  {((touchedConfirmEmal && !confirmEmail )||(alert  && !confirmEmail) )&&  (
      <div style={{ fontSize: '11px' }} className='text-red-500 font-semibold'>
        This field is required
      </div>
    )}
